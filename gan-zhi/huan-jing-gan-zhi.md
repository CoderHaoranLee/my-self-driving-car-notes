...
深度学习用于检测的一般的策略是现在原图像上提取一系列的框，然后将这些框中的图像送入CNN进行分类判别该框中是不是包含目标物体，并且对框的位置进行回归。常用的框提取算法包括Selective Search和EdgeBoxes。这些都是基于一些粗略的人工特征以及经验推断。
文献[126]中提出了一个能量最小化方法来提取框。通过在地平面上放置三维框，然后将其投影到图像上，通过计算一些简单的语义和物体实例分割，纹理，形状，位置先验等对这些框进行打分。
Faster-RCNN是第一个将框的提取以及检测放在同一个网络中并完成端到端的训练的深度学习检测算法。这个网络主要包含两个部分，提取网络和检测网络。这两个网络的卷积层的大部分都是共享的。提取网络的输出是提取出的框，这些框用于检测网络的输入进行分类以及对框的边界进行微调。
尽管Faster-RCNN的训练过程中没有固定框的大小，并且也假设框的大小是可变的，但是在物体大小变化较大的场景，一些小目标的检测效果不是特别好。主要原因是对有小的目标，通过不断的卷积层和池化层，目标在特征图上占据的数量越来越小，甚至于消失。一种解决方法是对原图进行尺度变换，输入时采用多尺度的图像，但是这样会增加计算量以及内存需求。
